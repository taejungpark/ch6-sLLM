    ...
    async def call_api(
        ...
        try:
            ...
            if "error" in response:
                ...
                err_code = str(response["error"].get("code", "")).lower()
                if any(s in err_msg for s in ("rate limit", "quota", "resource_exhausted", "rate_limit_exceeded")) or err_code in ("429", "resource_exhausted"):
...
def api_endpoint_from_url(request_url):
    """Extract the API endpoint (e.g. 'chat/completions') from the request URL.

    Supports OpenAI, Gemini OpenAI-compat (`/v1beta/openai/...`), Ollama
    (`http://localhost:11434/v1/...`), Groq, and Azure OpenAI deployments.
    """
    url = request_url.split("?")[0].rstrip("/")
    for suffix in ("chat/completions", "completions", "embeddings"):
        if url.endswith("/" + suffix):
            return suffix
    # Azure deployments fallback
    match = re.search(r"^https?://[^/]+/openai/deployments/[^/]+/(.+?)(\?|$)", request_url)
    if match:
        return match[1]
    raise ValueError(f"Could not infer API endpoint from URL: {request_url}")
